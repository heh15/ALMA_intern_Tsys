...
spw_autCorr = 19
if spw_autCorr == -1:
    average_autCorr = True
    spwautCorr_txt = 'avg'
else:
    average_autCorr = False
    spwautCorr_txt = str(spw_autCorr)
...
filename = 'Tsys_autCorr_correlation_spwautCorr'+spwautCorr_txt+'_spwTsys'+spwTsys_txt+'_avgTime'+timeAvg_txt+'.pkl' 
...
for iant in iants:
    ...
    autCorr_scans = dat.getcol('SCAN_NUMBER')
    tb.close()

    autCorr_sinspw = autCorr_avg    

    ## classify autCorr data into different types of observation
    elements = autCorr_scans

    test_elements = np.array(list(scan_phase)+list(np.intersect1d((scan_phase-1), scan_ATM)))
    isin_phase_autCorr = array_isin(elements, test_elements)
    autCorr_phase = autCorr_sinspw[isin_phase_autCorr]; time_autCorr_phase = autCorr_time[isin_phase_autCorr]

    test_elements = np.array(list(scan_sci)+list(np.intersect1d((scan_sci-1), scan_ATM)))
    isin_sci_autCorr = array_isin(elements, test_elements)
    autCorr_sci = autCorr_sinspw[isin_sci_autCorr]; time_autCorr_sci = autCorr_time[isin_sci_autCorr]

    test_elements = np.array(list(scan_bpass)+list(np.intersect1d((scan_bpass-1), scan_ATM)))
    isin_bpass_autCorr = array_isin(elements, test_elements)
    autCorr_bpass = autCorr_sinspw[isin_bpass_autCorr]; time_autCorr_bpass = autCorr_time[isin_bpass_autCorr]

    isin_obsrs_autCorr = [isin_phase_autCorr, isin_sci_autCorr, isin_bpass_autCorr]
    ...
    ### normalize the autCorr data based on the start of Tsys

    normScans = [phasenormScan, scinormScan, bpassnormScan]
    autCorr_means = np.full(np.shape(scan_ATM), fill_value=np.nan)
    autCorr_norms = np.full(np.shape(scan_ATM), fill_value=np.nan)

    isin_obsrs = [isin_phase, isin_sci, isin_bpass]
    for i, isin_obs in enumerate(isin_obsrs):
        if len(isin_obs) !=0:
            autCorr_time_obs = autCorr_time[isin_obsrs_autCorr[i]]
            autCorr_obs = autCorr_sinspw[isin_obsrs_autCorr[i]]
            ind = find_nearest(autCorr_time_obs, time_Tsys_avg[isin_obs])
            lowerTimes = autCorr_time_obs[ind]
            upperTimes = autCorr_time_obs[ind] + time_avg
            autCorr_matched, indices_matched = filter_data(autCorr_obs, autCorr_time_obs, lowerTimes, upperTimes) 
            autCorr_means[isin_obs] = np.array([np.mean(data) for data in autCorr_matched])
            autCorr_norms[isin_obs] = autCorr_means[isin_obs] / autCorr_means[isin_obs][normScans[i]]
    ...
    Tsys_table['autCorr_means'] = np.append(Tsys_table['autCorr_means'],autCorr_means)
    Tsys_table['autCorr_norms'] = np.append(Tsys_table['autCorr_norms'],autCorr_norms)
