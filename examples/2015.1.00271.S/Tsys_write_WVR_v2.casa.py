...
rmtables('uid___A002_Xbe0d4d_X12f5_v2.asdm.sdm.ms.tsys')
os.system('cp -r uid___A002_Xbe0d4d_X12f5.asdm.sdm.ms.tsys\
        uid___A002_Xbe0d4d_X12f5_v2.asdm.sdm.ms.tsys')
tb.open('uid___A002_Xbe0d4d_X12f5_v2.asdm.sdm.ms.tsys', nomodify=False)
nrows = tb.nrows()
...
for iant in iants:
    ...
    WVR_phase_norm = WVR_phase / WVR_phase[phase_ind0]
    ...
    tb.close()
    # trim the edge of the Tsys spectrum
#     Tsys_spec_orig[:,5:(np.shape(Tsys_spec_orig)[2]-5),:] = np.nan    

    Tsys_spec_ext = np.full(np.shape(Tsys_spectrum)[0:2]+\
                    np.shape(np.repeat(WVR_time_binned,4)), np.nan)
    for i in range(4):
        ...
        isin_sci_WVR_ext = isin_sci_WVR[0] + i * len(WVR_time_binned)
        isin_phase_WVR_ext = isin_phase_WVR[0] + i * len(WVR_time_binned)
        isin_bpass_WVR_ext = isin_bpass_WVR[0] + i * len(WVR_time_binned)
        Tsys_sci_ext = np.einsum('ij, k->ijk',
                Tsys_spec_orig[:,:,isin_sci_ext[scinormScan]], WVR_sci_norm)
        Tsys_spec_ext[:,:,isin_sci_WVR_ext] = Tsys_sci_ext
        Tsys_phase_ext = np.einsum('ij, k->ijk',
                Tsys_spec_orig[:,:,isin_phase_ext[phasenormScan]], WVR_phase_norm)
        Tsys_bpass_ext = np.einsum('ij, k->ijk',
                Tsys_spec_orig[:,:,isin_bpass_ext[bpassnormScan]], WVR_bpass_norm)
        Tsys_spec_ext[:,:,isin_bpass_WVR_ext] = Tsys_bpass_ext

    ## write a new Tsys into the alternative Tsys table
    # copy the information from .tsys table
    tb.open('uid___A002_Xbe0d4d_X12f5.asdm.sdm.ms.tsys')
    dat = tb.query('ANTENNA1==%d'%(iant))
    spws_Tsys_temp = dat.getcol('SPECTRAL_WINDOW_ID')

    # make the array for each column. 
    time_Tsys2 = np.tile(WVR_time_binned, 4)
    scans_Tsys2 = np.tile(WVR_scans_binned, 4)
    field_Tsys2 = np.tile(WVR_field_binned, 4)
    paramerr_Tsys2 = np.full(np.shape(Tsys_spec_ext), 0.1)
    spws_Tsys_temp = np.unique(spws_Tsys_temp)
    spws_Tsys2 = np.repeat(spws_Tsys_temp, len(WVR_time_binned))
    snr_Tsys2 = np.full(np.shape(Tsys_spec_ext), 1.0)
    flag_Tsys2 = np.full(np.shape(Tsys_spec_ext), False) 

    tb.open('uid___A002_Xbe0d4d_X12f5_v2.asdm.sdm.ms.tsys', nomodify=False)
    tb.addrows(len(time_Tsys2))
    tb.putcol('TIME',time_Tsys2,startrow) 
    tb.putcol('FIELD_ID',field_Tsys2,startrow) 
    tb.putcol('SPECTRAL_WINDOW_ID',spws_Tsys2, startrow) 
    tb.putcol('ANTENNA1', np.full(np.shape(time_Tsys2), iant), startrow)
    tb.putcol('ANTENNA2', np.full(np.shape(time_Tsys2), -1), startrow)
    tb.putcol('INTERVAL', np.full(np.shape(time_Tsys2), 0), startrow) 
    tb.putcol('SCAN_NUMBER', scans_Tsys2, startrow)
    tb.putcol('OBSERVATION_ID', np.full(np.shape(time_Tsys2), 0), startrow) 
    tb.putcol('FPARAM', Tsys_spec_ext, startrow) 
    tb.putcol('PARAMERR', paramerr_Tsys2, startrow) 
    tb.putcol('FLAG', flag_Tsys2, startrow) 
    tb.putcol('SNR', snr_Tsys2, startrow)
    tb.flush()
    tb.close()
    
    startrow = startrow + len(time_Tsys2)
...
# Tsys_test = np.nanmean(Tsys_temp, axis=0)[299:598]
# fig = plt.figure()
# # plt.scatter(WVR_time, Tsys_ext2)
# plt.scatter(WVR_time_binned, Tsys_test, color='red')
# plt.ylim(bottom=100)
# 
# # test 2
# fig = plt.figure()
...
# aU.plotbandpass(caltable='uid___A002_Xbe0d4d_X12f5_v2.asdm.sdm.ms.tsys', overlay='time',
#     xaxis='freq', yaxis='amp', subplot=22, buildpdf=False, interactive=True,
