...
Global_title = 'Sci Ant '+str(iant)
Global_filename = '_sci_ant'+str(iant)
...
dat = tb.query("ANTENNA1==%d && ANTENNA2==%d && DATA_DESC_ID in %s && SCAN_NUMBER in %s"%(iant,iant,ddid, list(scan_sci))) # 2 is scan id
...
ddid = spw_WVR
tb.open(vis)
dat = tb.query('ANTENNA1==%d && ANTENNA2==%d && DATA_DESC_ID==%d && SCAN_NUMBER in %s'%(iant,iant,ddid,list(scan_sci)))
...
ddid = spw_autCorr
tb.open(vis)
dat = tb.query('ANTENNA1==%d && ANTENNA2==%d && DATA_DESC_ID in %s && SCAN_NUMBER in %s'%(iant,iant,ddid,list(scan_sci)))
...
xdata = AutCorr_sinspw; xtime = AutCorr_time
ydata = sig_sinspw; ytime = sig_time
...
xdata = WVR_sinchan; ydata = AutCorr_regridded
...
plt.plot(xdata, ydata_fitted1, color='red', linestyle='dotted',
        ...
        +'\n err: '+str(round(fit_err_rel1[0], 6)))
plt.legend()
